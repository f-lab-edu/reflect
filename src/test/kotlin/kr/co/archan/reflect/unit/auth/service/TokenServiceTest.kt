...
class TokenServiceTest : BehaviorSpec({
    ...
    context("issueOnLogin - 로그인 시 AccessToken과 RefreshToken 발급") {
        Given("회원 정보가 주어지고") {
            ...
            When("토큰을 발급하면") {
                val result = tc.tokenService.issueToken(member)
    ...
    context("issueOnLogin - 서로 다른 이메일을 가진 회원에게 다른 토큰 발급") {
        Given("서로 다른 이메일을 가진 두 회원이 주어지고") {
            ...
            val member1 = Member.signUp("user1@example.com", tc.crypto.hashPassword("password1"), "사용자1")
            val member2 = Member.signUp("user2@example.com", tc.crypto.hashPassword("password2"), "사용자2")

            When("각각 토큰을 발급하면") {
                val result1 = tc.tokenService.issueToken(member1)
                val result2 = tc.tokenService.issueToken(member2)
    ...
    context("issueOnLogin - RefreshToken이 Repository에 저장됨") {
        Given("회원 정보가 주어지고") {
            ...
            val member = Member.signUp("test@example.com", tc.crypto.hashPassword("password"), "테스트")
    ...
    context("issueOnLogin - 같은 회원이 여러 번 로그인하면 매번 다른 토큰 발급") {
        Given("같은 회원이 주어지고") {
            ...
            val member = Member.signUp("repeat@example.com", tc.crypto.hashPassword("password"), "반복테스트")

            When("여러 번 토큰을 발급하면") {
                val result1 = tc.tokenService.issueToken(member)
                val result2 = tc.tokenService.issueToken(member)
                val result3 = tc.tokenService.issueToken(member)
    ...
    context("issueOnLogin - 토큰 만료 시간이 Properties 설정에 따라 올바르게 설정됨") {
        Given("회원 정보가 주어지고") {
            ...
            val member = Member.signUp("expiry@example.com", tc.crypto.hashPassword("password"), "만료테스트")
            val now = Instant.now()
